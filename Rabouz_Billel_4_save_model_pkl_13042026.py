...
save_model_pkl.py 
─────────────────
Corrections du tuteur :
  1. Filtre Outlier : garder uniquement les lignes sans outlier (NaN)
  2. Filtre ComplianceStatus : garder uniquement 'Compliant'
  3. Target : SiteEnergyUseWN(kBtu) au lieu de SiteEnergyUse(kBtu)
...
# ── 3. Filtre Outlier (garder uniquement NaN = pas d'outlier) ─────────────────
df = df[df["Outlier"].isna()].copy()
print(f"Après filtre Outlier       : {len(df)} bâtiments")

# ── 4. Filtre ComplianceStatus (garder uniquement 'Compliant') ────────────────
...
# Sauvegarder la target AVANT de supprimer
...
# ── 8. Création du DataFrame de modélisation ──────────────────────────────────
FEATURES_NUM = [
...
print("Lance maintenant : uvicorn service:app --reload --port 3000")
